...
| **H** | **Hierarchy & Boundary Clarity** | Leaks server logic into client bundles (`"use client"` abuse). | Strict RSC boundaries; data fetched server-side with typed contracts. |
...
| **S** | **Stack Native Strictness** | Next.js 12/13 legacy patterns; untyped `any` signatures. | Next.js 16 + React 19 native (`use`, Server Actions, strict TypeScript). |
...
| **V** | **Velocity & Performance** | Heavy waterfall requests, un-indexed queries, missing HMR hygiene. | Indexed queries, parallel `Promise.all`, optimized import boundaries. |
